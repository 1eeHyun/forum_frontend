...
export default function RelatedPostCard({ post }) {
  ...
  const media = post.fileUrls?.find((f) => f.type === "IMAGE" || f.type === "VIDEO");
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col justify-between text-sm text-muted w-full">
        {/* Top row: Tags or Community + Author */}
        ...
        <div className="text-xs text-muted mt-2 flex gap-4">
          <span>
            {post.likeCount} {POST_LABELS.LIKES}
          </span>
          <span>
            {post.commentCount} {POST_LABELS.COMMENTS}
          </span>
        </div>
      </div>
    </div>
