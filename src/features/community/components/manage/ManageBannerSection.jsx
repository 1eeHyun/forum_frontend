...
export default function ManageBannerSection({ community }) {
  ...
  const handleBannerChange = async (e) => {
    ...
    try {
      await axios.put(`/communities/${community.id}/banner-image`, formData, {
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      await axios.put(`/communities/${community.id}/profile-image`, formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
      window.location.reload();
    } catch (err) {
  ...
  return (
    <div className="relative h-[180px] md:h-[220px] rounded-2xl overflow-hidden border border-gray-700 mb-12 group">
      {/* Banner image */}
      <div
      ...
      >
        <div className="text-white bg-black/50 rounded-lg px-4 py-2 flex items-center gap-2 hover:bg-black/70">
          <Camera size={18} />
        ...
        >
          ...
          >
            <div className="rounded-full overflow-hidden w-full h-full border-4 border-white cursor-grab">
              <img
                src={profileFile instanceof File ? URL.createObjectURL(profileFile) : ""}
                alt="preview"
                draggable={false}
      ...
      <div className="absolute left-6 bottom-4 group/profile z-40">
        <div className="relative w-20 h-20 md:w-24 md:h-24 rounded-full border-4 border-white bg-white overflow-hidden">
          <img
